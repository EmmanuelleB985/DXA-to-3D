...
class DXADataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Prepare sample
        sample = {"image": image, **annotations, "metadata": sample_info.get("metadata", {})}
    ...
    def _resample_curve(self, points: np.ndarray, target_points: int) -> np.ndarray:
    ...
    def _normalize_points(self, points: np.ndarray, scale: float = 1.0) -> np.ndarray:
...
def get_data_transforms(
    training: bool = True, target_size: Tuple[int, int] = (512, 512)
) -> A.Compose:
