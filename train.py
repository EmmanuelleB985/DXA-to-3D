...
from model import SpineShapeLoss, create_model
from utils import (
    EarlyStopping,
    MetricTracker,
    ModelCheckpoint,
...
class Trainer:
    ...
    def __init__(
        ...
        self.early_stopping = EarlyStopping(
            patience=config.early_stopping_patience, min_delta=config.early_stopping_delta
        )
    ...
    def _create_optimizer(self) -> torch.optim.Optimizer:
        ...
        optimizer_class = optimizer_map.get(self.config.optimizer.name.lower(), optim.AdamW)
    ...
    def _create_scheduler(self) -> Optional[torch.optim.lr_scheduler._LRScheduler]:
        ...
        scheduler_class = scheduler_map.get(
            self.config.scheduler.name.lower(), optim.lr_scheduler.CosineAnnealingLR
        )

        return scheduler_class(self.optimizer, **self.config.scheduler.kwargs)
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        pbar = tqdm(
            self.train_loader, desc=f"Epoch {epoch}/{self.config.num_epochs} [Train]", leave=False
        )

        for batch_idx, batch in enumerate(pbar):
            # Move data to device
            images = batch["image"].to(self.device)
            targets = {key: val.to(self.device) for key, val in batch.items() if key != "image"}
            ...
            # Update progress bar
            pbar.set_postfix(
                {"loss": f"{loss.item():.4f}", "lr": f"{self.optimizer.param_groups[0]['lr']:.6f}"}
            )

        return epoch_metrics.get_averages()

    def _validate_epoch(self, epoch: int, final: bool = False) -> Dict[str, float]:
        ...
        with torch.no_grad():
            pbar = tqdm(
                self.val_loader, desc=f"Epoch {epoch}/{self.config.num_epochs} [Val]", leave=False
            )
    ...
    def _log_metrics(
        ...
        if self.config.use_wandb:
            log_dict = {"epoch": epoch, "learning_rate": self.optimizer.param_groups[0]["lr"]}
    ...
    def _visualize_batch(
        self, predictions: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor], epoch: int
    ) -> None:
        ...
        for idx in range(min(4, pred_np["coronal_centerline"].shape[0])):
            fig = visualize_predictions(pred_np, target_np, idx=idx)
...
def main(config: DictConfig) -> None:
    ...
    # Create model
    model = create_model(model_type=config.model.type, **config.model.params)
    ...
    # Create trainer
    trainer = Trainer(
        config=config, model=model, train_loader=train_loader, val_loader=val_loader, device=device
    )
