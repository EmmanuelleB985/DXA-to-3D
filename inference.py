...
class SpineEstimator:
    ...
    def __init__(self, config: InferenceConfig, device: Optional[torch.device] = None) -> None:
    ...
    def _load_model(self) -> nn.Module:
        ...
        # Create model
        model = create_model(model_type=model_config.type, **model_config.params)
    ...
    def predict_single(
        self, image_path: str, visualize: bool = True, save_results: bool = True
    ) -> Dict[str, np.ndarray]:
    ...
    def predict_batch(
        self, image_paths: List[str], batch_size: Optional[int] = None
    ...
    def predict_directory(self, input_dir: Optional[str] = None) -> List[Dict[str, np.ndarray]]:
    ...
    def _save_results(self, results: Dict[str, Any], sample_id: str) -> None:
        ...
        if "spine_3d" in results and self.config.save_3d_mesh:
            save_3d_spine_mesh(results["spine_3d"], str(sample_dir / "spine_3d.ply"))

    def _visualize_results(self, results: Dict[str, Any], sample_id: str) -> None:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="3D Spine Shape Estimation Inference")
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument("--input", type=str, help="Input image or directory (overrides config)")
    ...
    parser.add_argument(
        "--batch-size", type=int, help="Batch size for inference (overrides config)"
    )
